  ...
  absl::StatusOr<Value> LowerConvert(ImplicitLocOpBuilder& builder,
    ...
    if (dst_complex_element_ty) {
      ...
      if (auto src_complex_elem_ty =
        ...
        Type real_src_elem_ty = src_complex_elem_ty.getElementType();
        Type real_src_ty = src_ty;
        if (auto shaped_ty = mlir::dyn_cast<ShapedType>(src_ty)) {
          real_src_ty = shaped_ty.clone(real_src_elem_ty);
        } else {
          real_src_ty = real_src_elem_ty;
        }
        Value real_input =
