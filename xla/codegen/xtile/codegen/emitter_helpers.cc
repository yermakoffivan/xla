...
absl::StatusOr<mlir::TypedValue<mlir::RankedTensorType>> EmitConstant(
  ...
  if (primitive_util::IsComplexType(constant.shape().element_type())) {
    if (constant.shape().element_type() == C64) {
      std::complex<float> val = constant.literal().GetFirstElement<complex64>();
      return CreateConst(b, ty, val, shape);
    }
    std::complex<double> val = constant.literal().GetFirstElement<complex128>();
    return CreateConst(b, ty, val, shape);
  }
