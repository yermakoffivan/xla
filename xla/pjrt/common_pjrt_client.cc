...
absl::StatusOr<std::unique_ptr<PjRtBuffer>> CommonPjRtClient::MakeUndonatable(
  ...
  if (!common_pjrt_buffer) {
    return absl::InvalidArgumentError(
        "MakeUndonatable called on a non-CommonPjRtBuffer.");
  }
  std::shared_ptr<const Shape> on_device_shape =
      std::make_shared<const Shape>(buffer->on_device_shape());
  PjRtMemorySpace* memory_space = buffer->memory_space();

  std::unique_ptr<UndonatableCommonPjRtBuffer> undonatable_buffer;
  RETURN_IF_ERROR(common_pjrt_buffer->AcquireScopedRawBuffer(
      [&](PjRtRawBufferRef raw_buffer,
        ...
        for (size_t i = 0; i < definition_events.size(); ++i) {
          events.push_back(definition_events[i].CopyRef());
        }
        undonatable_buffer = std::make_unique<UndonatableCommonPjRtBuffer>(
            std::move(on_device_shape), std::move(raw_buffer),
            std::move(events), memory_space);
