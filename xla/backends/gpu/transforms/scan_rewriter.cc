...
absl::StatusOr<bool> ScanRewriter::RunOnComputation(
  ...
  for (HloInstruction* inst : computation->instructions()) {
    if (hlo_query::IsStandardAssociativeScan(inst, /*allow_reverse=*/true,
                                             /*allow_used_carry=*/true)) {
  ...
  for (HloScanInstruction* scan : scans) {
    ...
    HloInstruction* seed = scan->mutable_operand(1);
    ...
    // so live carry users can be fed a slice of the output.
    ...
    HloInstruction* carry_slice = computation->AddInstruction(
        HloInstruction::CreateSlice(slice_shape, out, starts, limits, strides));
    HloInstruction* carry =
        computation->AddInstruction(HloInstruction::CreateReshape(
            scan->shape().tuple_shapes(1), carry_slice));
    HloInstruction* replacement =
        computation->AddInstruction(HloInstruction::CreateTuple({out, carry}));
