...
ENTRY entry {
  ...
  EXPECT_THAT(
      ...
      ::xla::GmockMatch(m::GetTupleElement(
          m::Tuple(m::Add(m::GetTupleElement(m::CustomCall(m::Parameter(0)), 0),
...
ENTRY entry {
  ...
  EXPECT_THAT(
      module->entry_computation()->root_instruction(),
      ::xla::GmockMatch(m::GetTupleElement(
          m::Tuple(m::GetTupleElement(m::CustomCall(m::Parameter(0)), 0),
                   m::Reshape(m::Slice(
...
ENTRY entry {
  ...
  // A reverse scan's final carry is its first output element.
  const HloInstruction* scan_replacement =
      module->entry_computation()->root_instruction()->operand(0)->operand(0);
  EXPECT_THAT(scan_replacement,
              ::xla::GmockMatch(m::Tuple(
                  m::Add(m::GetTupleElement(m::CustomCall(m::Parameter(0)), 0),
                         m::Broadcast(m::Parameter(1))),
                  m::Reshape(m::Slice(m::Add(
                      m::GetTupleElement(m::CustomCall(m::Parameter(0)), 0),
                      m::Broadcast(m::Parameter(1))))))));
  const HloInstruction* slice = scan_replacement->operand(1)->operand(0);
